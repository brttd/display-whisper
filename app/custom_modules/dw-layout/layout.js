...
//Menu methods
{
    ...
    */
    ...
    exports.menu = {}
    {
        ...
        //thisWin.setMenu(null)

        function getObjFromItem(item) {
            return {
                topContext: item.topContext,
        ...
        function onItemClick(item) {
            if (typeof item.window === 'string') {
                ipcRenderer.send('open-window', item.window)
                return false
                ...
                if (Array.isArray(item.value)) {
                    ipcRenderer.send(item.message, ...item.value)
                } else {
                    ipcRenderer.send(item.message, item.value)
                }
            ...
            if (item.label.toLowerCase() === 'open devtools...') {
            ...
            if (customEvent.topContext) {
                if (Array.isArray(listeners[customEvent.topContext])) {
                    for (
                        let i = 0;
                        i < listeners[customEvent.topContext].length;
                        i++
                    ) {
                        listeners[customEvent.topContext][i](customEvent)
                    }
                }
            }
        ...
        function findMenuItem(toSearch, obj) {
            for (let i = 0; i < toSearch.items.length; i++) {
                if (
                    toSearch.items[i].label === obj.label &&
                    toSearch.items[i].accelerator === obj.accelerator
                ) {
                    return toSearch.items[i]
                }

                if (toSearch.items[i].submenu) {
                    let found = findMenuItem(toSearch.items[i].submenu, obj)
                    if (found) {
                        return found
                    }
                }
        ...
        function onItemAccelerator(obj) {
            let item = findMenuItem(menuBar, obj)
        ...
        function registerAccelerators(item) {
            ...
            if (Array.isArray(item.submenu)) {
                for (let i = 0; i < item.submenu.length; i++) {
                    registerAccelerators(item.submenu[i])
                }
            }

            if (typeof item.accelerator === 'string') {
                //TODO: is this the best idea? Should only remove previously registered menu shortcuts, not all shortcuts...
                keyboard.clear(item.accelerator)

                keyboard.register(
                    item.accelerator,
                    () => {
                        onItemAccelerator(item)
                    },
                    {
                        global: false
                    }
        ...
        Object.defineProperty(exports.menu, 'items', {
            get: () => {
                return menuBar.items
            }
        })

        exports.menu.set = function(items) {
            ...
            if (insertedMenu) {
                menuBar.append(insertedMenu)
            }

            for (let i = 0; i < items.length; i++) {
                exports.menu.add(items[i])
            }
        }

        exports.menu.add = function(item) {
            //TODO: create wrapper class for menu items which gives a more user friendly API for modifiying menu items
            //(Like for adding/removing child items, etc)
            let returnVal = null

            if (typeof item === 'string') {
                if (presets.hasOwnProperty(item)) {
                    presets[item] = cleanItem(presets[item])

                    usedPresets[item] = new MenuItem(presets[item])

                    registerAccelerators(presets[item])

                    menuBar.append(usedPresets[item])

                    returnVal = usedPresets[item]
                }
            } else if (
                typeof item === 'object' &&
                item !== null &&
                ...
                returnVal = new MenuItem(item)

                menuBar.append(returnVal)

                registerAccelerators(item)
            }

            if (process.platform !== 'darwin') {
                thisWin.setMenu(menuBar)
            }

            return returnVal
        }

        exports.menu.change = function(item, change) {
            if (usedPresets.hasOwnProperty(item)) {
                switch (item) {
                    case 'edit':
                        if (typeof change.undo === 'boolean') {
                            usedPresets[item].submenu.items[0].enabled =
                                change.undo
                        }
                        if (typeof change.redo === 'boolean') {
                            usedPresets[item].submenu.items[1].enabled =
                                change.redo
                        }

                        break
                }
            } else if (typeof item.commandId === 'number') {
                if (
                ...
                change.topContext = change.topContext || item.topContext
                change.context = change.context || item.context

                change = cleanItem(change)

                //label, type, role, etc cannot be changed
                if (!Array.isArray(change.submenu) || !item.submenu) {
                    return false
                }

                //TODO: this is all custom code for changing the recently opened file menu
                //It really needs to be changed
                //Since no other window has need to change submenus, it works at the moment

                let newIndex = change.submenu.length - 1
                let currentIndex = item.submenu.items.length - 1

                while (newIndex >= 0 && currentIndex >= 0) {
                    item.submenu.items[currentIndex].visible = false
                    item.submenu.items[currentIndex].enabled = false

                    if (
                        item.submenu.items[currentIndex].label ===
                        change.submenu[newIndex].label
                    ) {
                        item.submenu.items[currentIndex].visible = true
                        item.submenu.items[currentIndex].enabled = true
                ...
                for (let i = currentIndex; i >= 0; i--) {
                    ...
                    item.submenu.items[i].enabled = false
                }

                while (newIndex >= 0) {
                    item.submenu.insert(
                        0,
                        new MenuItem(change.submenu[newIndex])
                    )
        ...
        exports.menu.onEvent = function(name, listener) {
